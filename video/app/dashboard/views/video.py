...
from app.utils.permission import dashboard_aush
...
class ExternaVideo(View):
    ...
    def post(self, request):
        ...
        if not all([name, image, video_type, from_to, nationality, info]):
            return redirect('{}?error={}'.format(reverse('externa_video'), '缺少必要信息'))

        result = check_and_get_video_type(VideoType, video_type, '非法的视频类型')
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse('externa_video'), result.get('msg')))
